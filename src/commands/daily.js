...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('daily')
        .setDescription('Claim your Hyperion daily reward!'),
    async execute(interaction) {
        ...
        if (lastDaily && (now.getTime() - lastDaily.getTime()) < cooldown) {
            ...
            const container = new ContainerBuilder().setAccentColor(0xFF4444)
                .addSectionComponents(new SectionBuilder().addTextDisplayComponents(
                    new TextDisplayBuilder().setContent(`❌ **Cooldown Active**\nYou've already claimed your reward. Come back in **${hours}h ${minutes}m**.`)));
            return interaction.reply({ ...container.toJSON(), ephemeral: true });
        }

        // Calculate streak (broken if > 48 hours since last)
        ...
        const container = new ContainerBuilder().setAccentColor(0x2ECC71)
            .addSectionComponents(new SectionBuilder().addTextDisplayComponents(
                new TextDisplayBuilder().setContent(`✅ **Daily Reward Received!**\nYou claimed **${totalReward}** ${getEmoji('COIN')}!\n🔥 Current Streak: **${streak}**`)));
